use hekate_groestl::{compress_2to1, compress_2to1_prp};
use hekate_math::{Block128, HardwareField, TowerField};

#[test]
fn compress_2to1_is_deterministic() {
    let left = [
    ...
    let a1 = compress_2to1(left, right);
    let a2 = compress_2to1(left, right);
    assert_eq!(a1, a2);

    let b1 = compress_2to1_prp(left, right);
    let b2 = compress_2to1_prp(left, right);
    assert_eq!(b1, b2);
}

#[test]
fn compress_2to1_is_order_sensitive() {
    let left = [
    ...
    let a_lr = compress_2to1(left, right);
    let a_rl = compress_2to1(right, left);
    assert_ne!(a_lr, a_rl);

    let b_lr = compress_2to1_prp(left, right);
    let b_rl = compress_2to1_prp(right, left);
    assert_ne!(b_lr, b_rl);
}

#[test]
fn compress_2to1_variants_are_domain_separated() {
    let left = [
    ...
    let a = compress_2to1(left, right);
    let b = compress_2to1_prp(left, right);
