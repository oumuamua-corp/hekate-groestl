...
Hardware-accelerated, algebra-native hash function optimized for GKR recursion over Binary Tower Fields $GF(2^{128})$.

`hekate-groestl` is a high-throughput variant of the [Groestl](https://www.groestl.info/) permutation, re-engineered for
Arithmetic Circuits. It serves as the cryptographic backbone for the Hekate ZK Engine, leveraging the linear properties
of binary fields to minimize prover overhead while maximizing native CPU execution via SIMD intrinsics.
